...
//	}
	...
	public FileSystemStorageServiceImpl(StorageProperties props) {
		org.springframework.util.Assert.hasText(props.getLocation(), "storage.location must not be empty");
		this.rootLocation = java.nio.file.Paths.get(props.getLocation()).toAbsolutePath().normalize();
		try {
			java.nio.file.Files.createDirectories(this.rootLocation);
		} catch (java.io.IOException e) {
			throw new RuntimeException("Init upload dir failed: " + this.rootLocation, e);
		}
	}
